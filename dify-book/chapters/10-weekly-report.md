...
::: tip 进阶：用 Schedule Trigger 实现全自动周报
目前这个 Workflow 还需要手动粘贴数据、手动点运行。如果数据源有 API 接口（比如公司内部系统），你可以：
1. 把"开始"节点切换为 **Schedule Trigger**，设置为每周五下午 3 点自动触发
2. 用 HTTP 请求节点自动从内部系统拉取数据
3. 配合邮件或 Slack 插件，生成完自动发送

这样就彻底不用人参与了。Trigger 的具体用法见第 18 章。
:::

...
>
> ```
> 客户：I'm looking for a new phone.
> AI：Sure! What's your budget?
> 客户：Around $500.
> AI：What kind of product are you looking for?（完全没记住预算）
> ```
>
...
| **内置变量** | `sys.user_id`、`sys.files` 等基础变量 | 在此基础上增加 `sys.query`（当前用户输入）、`sys.conversation_id` 等 |
| **会话变量** | 不支持 | 支持，跨轮次持久化 |
...
客户：预算 $500 左右，主要拍照好的
AI：$500 价位拍照出色的话，推荐我们的 TechPhone Pro 16（$599 起）。
    它搭载 5000 万主摄 + 6400 万长焦，支持 Leica 光学……
    如果预算想控制更紧一些，TechPhone Lite 12 的拍照也不错。
    您想了解哪款的详细配置？
...
小林笑了。折腾了这么多章，TechStore 的 AI 客服总算像个正经客服了。

客服的活到这里告一段落。接下来，小林要对付第二个痛点——每周五下午的运营周报。

...
1. **知识是静态的**——训练数据有截止日期，不知道今天发生了什么
2. **只能"说"不能"做"**——能告诉你怎么查快递，但不能真的去查

**工具调用（Tool Calling）** 解决了这两个问题。
...
**自定义工具：**
- 通过 HTTP Request 调用任意 API
- 通过代码节点实现复杂逻辑

## 第一步：安装搜索工具

1. 点击 Dify 顶部导航的「插件」或「工具」
2. 在工具列表中找到「DuckDuckGo」（不需要 API Key，推荐新手使用）
3. 点击「安装」

安装完成后，这个工具就可以在 Workflow 中使用了。

## 第二步：添加 Agent 节点

Agent 节点是一个特殊的 LLM 节点，它可以**自主决定是否调用工具**。
...
**阶段成果**：TechStore 客服从简单的 FAQ 机器人，进化为能分类、能拆解、能查询的智能助手。

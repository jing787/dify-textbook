...
在 Dify 中，你可以创建五种类型的应用：

| 类型 | 说明 | 适用场景 |
|------|------|----------|
| **Chatbot** | 对话式助手，一问一答 | 简单问答、闲聊 |
| **Text Generator** | 文本生成器，输入→输出 | 翻译、摘要、改写 |
| **Agent** | 智能体，能自主调用工具 | 需要查询外部信息的场景 |
| **Workflow** | 工作流，多步骤流程编排 | 复杂业务逻辑 |
| **Chatflow** | 对话式工作流，Workflow + 对话记忆 | 多轮对话的复杂场景 |

小林目前用的是 **Chatbot**——最简单的类型。它的能力边界很明显：
- 只能根据知识库回答
- 不能分步骤处理
- 不能调用外部系统
- 不能根据问题类型走不同流程
